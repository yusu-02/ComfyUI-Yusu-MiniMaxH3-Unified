...
- 参考音频不再被插件额外限制为最多 15 秒，上传波形与官方 `LoadAudio` 节点保持一致。
...
- 每段参考视频：2–15 秒。
- 所有参考视频总时长：不超过 15 秒。
- 参考音频时长与官方 `MiniMaxH3ReferenceToVideo` 保持一致，不由插件额外截断。
- 生成宽高必须是 32 的倍数。
- 单个上传文件最大 512 MiB。
- 图片最大 100,000,000 像素。
- 参考音频采样率最高 384 kHz。

这些是参考媒体限制，与不设上限的输出 `duration` 是两套不同规则。较长参考音频会增加 Audio VAE 的显存与处理时间。
