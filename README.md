...
- 上传改用 ComfyUI 标准 `/upload/image` 资产入口，修复 RunningHub 设计端上传成功、执行容器却找不到媒体文件的问题。
- 支持 RunningHub 返回的 `api/哈希.扩展名` 等标准输入相对路径，同时继续拒绝绝对路径、父目录和 annotated 跨目录路径。
- 波形画布不再把点击事件交给裁剪时间轴，修复点击“加载波形”反而移动裁剪标尺的问题。
- 兼容 RunningHub 未展开 Autogrow 的前端，将灰色媒体组接口转换为可连接的 `IMAGE` / `AUDIO` 子接口。
- 更新插件后必须完整重启 ComfyUI/RunningHub 工作区；若前后端版本错位，上传时会明确提示重启。
...
RunningHub 可能改存为 `ComfyUI/input/api/哈希.扩展名`，插件以标准上传接口返回的实际相对路径为准。
...
- 前端网络请求仅访问当前 ComfyUI/RunningHub 工作区的标准上传、插件媒体检查和 `/view` 路由。
